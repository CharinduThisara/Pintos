...
void terminal(void){
  while(1){
	  ...
	  if (strcmp(input, "whoami") == 0){
	  	printf ("CS2042> ");
	  	printf("I.M.C. Thisara 200652D\n");
	  }
	  else if (strcmp(input, "shutdown")==0){
	  	break;
	  }	
	  else if (strcmp(input, "time")== 0){
	  	printf ("\nCS2042> %lus",rtc_get_time());
	  	
	  }else if (strcmp(input, "ram")== 0){
	  	printf ("\nCS2042> %dKB",init_ram_pages*4);
	
	  }else if (strcmp(input, "thread")== 0){
	  	thread_print_stats();
	  }else if (strcmp(input, "priority")== 0){
	  	printf ("\nCS2042> %i",thread_get_priority());
...
pintos_init (void)
{
  ...
  if (*argv != NULL) {
    ...
    run_actions (argv);
    printf ("command args. avail.\n");
  } else {
    // TODO: no command line passed to kernel. Run interactively 
    printf ("No command args.\n");
    terminal();
